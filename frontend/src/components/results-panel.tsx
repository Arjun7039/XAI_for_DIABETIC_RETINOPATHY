...
export function ResultsPanel({ result, rejection, onRetakeRequested }: ResultsPanelProps) {
  ...
  const isSevere = result.class_index === 3 || result.class_index === 4;
  const severity = SEVERITY_CONFIG[result.class_index] || SEVERITY_CONFIG[2];

  const formattedPrediction = result.prediction.includes("No_DR") || result.prediction === "No DR"
    ? "No DR (Healthy)"
